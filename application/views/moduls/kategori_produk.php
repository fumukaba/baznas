...
<script>
	...
	function save() {
			...
			var url;
			if (save_method == 'add') {
				url = "<?php echo site_url('Kategori_produk')?>/ajax_add";
			} else {
				url = "<?php echo site_url('Kategori_produk')?>/ajax_update"; 
			}
	...
	function edit(id) {
			...
			document.getElementById('formAksi').reset();
			$.ajax({
				url : "<?php echo site_url('Kategori_produk')?>/ajax_edit/"+id,
				type: "GET",
				dataType: "JSON",
				success: function(result) {  
					//document.getElementById('fc_kdbahan').setAttribute('readonly','readonly');
					$('[name="id_kategori_produk"]').val(result.fc_id);
					$('[name="nama_kategori_produk"]').val(result.fv_nama_kategori);
...
<form class="form-horizontal" role="form" id="formAksi">
	...
	<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Nama Kategori Produk</label>
		<div class="col-sm-10">
			<input type="text" id="nama_kategori_produk" name="nama_kategori_produk" placeholder="Nama Kategori Produk" class="col-xs-10 col-sm-5" />
		</div>
	</div>
