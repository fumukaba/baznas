...
<script>
	var save_method;
	var link = "<?php echo site_url('User_type')?>";
	var table;
	...
	$(document).ready(function() {
        ...
        "ajax": {
            "url": "<?php echo site_url('User_type/ajax_list')?>",
            "type": "POST"
        },
	...
	function save() {
			...
			var url;
			if (save_method == 'add') {
				url = "<?php echo site_url('User_type')?>/ajax_add";
			} else {
				url = "<?php echo site_url('User_type')?>/ajax_update"; 
			}
	...
	function edit(id) {
			...
			document.getElementById('formAksi').reset();
			$.ajax({
				url : "<?php echo site_url('User_type')?>/ajax_edit/"+id,
				type: "GET",
				dataType: "JSON",
				success: function(result) {  
	...
	function hapus(id) {
		if (confirm('Are you sure delete this data?')) {
			$.ajax ({
				url : "<?php echo site_url('User_type/ajax_delete')?>/"+id,
				type: "POST",
				dataType: "JSON",
...
<div class="widget-box">
<div class="widget-header">
		<h4 class="widget-title">Form Entry Level User</h4>
