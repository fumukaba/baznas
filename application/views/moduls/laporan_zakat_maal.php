...
<form action="<?php echo base_url('Laporan_zakat_maal/filter'); ?>" method="POST">
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="input-group date"> 
				<label for="">Status Maal</label>
				<select name="status_maal" id="status_maal" autocomplete="off">
					<option value="" selected="selected">Semua status maal</option>
		...
		<div class="col-lg-12">
			<div class="input-group date">
				...
				<select name="status_uang" id="status_uang" autocomplete="off">
					<option value="" selected="selected">Semua status uang</option>
		...
		konfirmasiStatus = function(_this) {
			...
			if(konfirmasi == 'tidak') {
				$.ajax({
					type: 'POST',
					url: url,
					data: 'id_infaq=' + id_infaq + '&status_maal=Tidak Valid&jumlah_infaq=' + jumlah,
					dataType: 'json',
					processData:false,
					success: function(result) {
						if (result.status) {
