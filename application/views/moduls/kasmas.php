...
<?php $title = "<i class='fa fa-user'></i>&nbsp;Kas Masuk"; ?>
...
<script>
	var save_method;
	var link = "<?php echo site_url('Kas_masuk')?>";
	var table;
	...
	$(document).ready(function() {
        ...
        "ajax": {
            "url": "<?php echo site_url('Kas_masuk/ajax_list')?>",
            "type": "POST"
        },
	...
	function save() {
			...
			var url;
			if (save_method == 'add') {
				url = "<?php echo site_url('Kas_masuk')?>/ajax_add";
			} else {
				url = "<?php echo site_url('Kas_masuk')?>/ajax_update"; 
			}
	...
	function edit(id) {
			...
			document.getElementById('formAksi').reset();
			$.ajax({
				url : "<?php echo site_url('Kas_masuk')?>/ajax_edit/"+id,
				type: "GET",
				dataType: "JSON",
				success: function(result) {  
	...
	function hapus(id) {
		if (confirm('Are you sure delete this data?')) {
			$.ajax ({
				url : "<?php echo site_url('Kas_masuk/ajax_delete')?>/"+id,
				type: "POST",
				dataType: "JSON",
